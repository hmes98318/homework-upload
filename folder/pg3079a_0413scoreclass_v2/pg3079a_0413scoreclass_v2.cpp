...
int Max_math(vector<score> &students, int size)
{
    int maxIndex = 0, maxMath = 0;
...
int Max_english(vector<score> &students, int size)
{
    int maxIndex = 0, maxEnglish = 0;
    for (int i = 0; i < size; i++)
    {
...
int Max_application(vector<score> &students, int size)
{
    int maxIndex = 0, maxAppli = 0;
    for (int i = 0; i < size; i++)
    {
        if (students[i].application > maxAppli)
            maxIndex = i;
    }
    return maxIndex;
}

int Max_oral(vector<score> &students, int size)
{
    int maxIndex = 0, maxOral = 0;
    for (int i = 0; i < size; i++)
    {
        if (students[i].oral > maxOral)
            maxIndex = i;
    }
    return maxIndex;
}
...
int main()
{
    ...
    int max_math= Max_math(students, size);
    int max_english = Max_english(students, size);
    int max_application= Max_application(students, size);
    int max_oral = Max_oral(students, size);
    ...
    int sameEnglish = english[0].english;
    ...
    int sameApplication = application[0].application;
    ...
    cout<<"max_math : "<<max_math<<"\tmax_english : "<<max_english<<"\tmax_application : "<<max_application<<"\tmax_oral : "<<max_oral<<endl;
    cout << "Best : " << oral[0].name << "\t" << oral[0].math << "\t" << oral[0].english << "\t" << oral[0].nature << "\t" << oral[0].society << "\t" << oral[0].chinese << "\t" << oral[0].application<<"\t"<<oral[0].oral << endl;
